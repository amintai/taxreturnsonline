...
import { MessageCircle, User, Smartphone, MapPin, Building2 } from "lucide-react";
...
const CompanySideForm = () => {
  const [form, setForm] = useState({ name: "", mobile: "", state: "", companyType: "", terms: false });
  ...
  const handleSubmit = (e) => {
    ...
    const text = [
      ...
      `*Mobile:* ${form.mobile}`,
      `*State:* ${form.state}`,
      form.companyType ? `*Company Type:* ${form.companyType}` : null,
    ].filter(Boolean).join("\n");

    window.open(`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(text)}`, "_blank");
    setDone(true);
    setForm({ name: "", mobile: "", state: "", companyType: "", terms: false });
  };
      ...
      <div className="p-5">
        {done ? (
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700 mb-1">
                <Building2 size={14} className="text-gray-400" /> Company Type
                <span className="text-gray-400 text-xs ml-1">(optional)</span>
              </label>
              <select name="companyType" value={form.companyType} onChange={handleChange} className={inputClass("companyType")}>
...
export default CompanySideForm;
